...
How to get Scriptly up and running on a DO box. I followed these instructions on a Ubuntu 14.04 LTS box. For security purposes we want it to be running only on HTTPS.

Download Scriptly
---------------

Download and install Scriptly, by any of the standard methods. For this tutorial this was run in the home directory i.e. /home/user/
...
Getting Scriptly to listen on Port 80
-------------------

You could at this point have Scriptly listen to port 80 (assuming it's open) with the following command, run from the folder containing manage.py:
...
Getting Scriptly to talk to Nginx & uWSGI
---------------------

We will basically follow the tutorial `here
<http://uwsgi-docs.readthedocs.io/en/latest/tutorials/Django_and_nginx.html>`_. If you installed Scriptly in a virtualenv from the get go, then follow all those steps, if not you can ignore that set:
...
2. Test your Django project
    ...
    * In order to get this step to work correctly I found I had to call uWSGI from the main folder where manage.py lives, in my case that was /home/user/ProjectName (I have no idea why, but otherwise it won't find the Scriptly project correctly)
...
If you want to use a file socket, I then created an empty file in the Scriptly project directory to be used as one (in this example named django.sock).
...
At this point we should now have Scriptly running on Port 80 through Nginx.
