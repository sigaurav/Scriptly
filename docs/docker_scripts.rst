...
.. code-block:: python
    ...
    if __name__ == "__main__":
        ...
        scriptly_data_dir = os.getcwd()
        print(
            client.containers.run(
                image="busybox",
                command=f"dd if=/dev/urandom of=/output/test.garbage bs=1M count=1",
                mounts=[Mount(target="/output", source=scriptly_data_dir, type='bind')],
                ).decode(
                "utf-8"
            )
        )

If you are running Scriptly inside Docker, then this becomes a bit more complicated as the data directory may
...
.. code-block:: python
    ...
    if __name__ == "__main__":
        args = parser.parse_args()
        client = docker.from_env()
        # our volume is called `scriptly_user_uploads`. If we had a NAS, then we could simplify this by ensuring the path
        # inside the container matches the path on the host -- in which case the above snippet is all that is needed.
        volume = client.volumes.get('scriptly_user_uploads')
        volume_mount = volume.attrs['Mountpoint']
        current_dir = os.getcwd()
        scriptly_data_dir = os.path.join(volume_mount.rstrip('/'), current_dir.replace(settings.MEDIA_ROOT, '').lstrip('/'))
        print(
            client.containers.run(
                image="busybox",
                command=f"dd if=/dev/urandom of=/output/test.garbage bs=1M count=1",
