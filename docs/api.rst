...
Scriptly's API allows for programmatic access to manage scripts as well as submit and query jobs.
All use of the API requires a user to be authenticated via :ref:`API keys <api_keys>` and
the API to be enabled by setting `SCRIPTLY_ENABLE_API_KEYS` in your user_settings.py file.
...
.. code-block:: python
    ...
    response = requests.post(
      'https://scriptly.fly.dev/api/scripts/v1/add-or-update/',
      data={
...
.. code-block:: python
    ...
    response = requests.post(
      'https://scriptly.fly.dev/api/scripts/v1/cat-fetcher/submit/',
      data={
        "job_name": "test job",
...
.. code-block:: python
    ...
    response = requests.post(
      'https://scriptly.fly.dev/api/scripts/v1/protein-translation/submit/',
      data={
        "job_name": "test job",
...
currently does not exist for Scriptly's API as there is no public access permitted. **Importantly, these requests
are GET requests**
...
.. code-block:: python
    ...
    requests.get(
      'https://scriptly.fly.dev/api/jobs/v1/123/status/',
      headers={'Authorization': 'Bearer your_token_here'},
    )
    ...
    requests.get(
      'https://scriptly.fly.dev/api/jobs/v1/123/details/',
      headers={'Authorization': 'Bearer your_token_here'},
    )
    # {
    #    "status": "completed",
