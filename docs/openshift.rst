...
In the same local environment you cloned from, you can installed Scriptly via:

::

    pip install scriptly

or if you want to be on the bleeding-edge for some reason, from the github repo by:

::

    pip install git+https://github.com/scriptly/Scriptly.git



Bootstrapping Scriptly
-------------------

Next, you want to cd into your project root, which should be called wsgi. Here, you want to run to scriptly bootstrapper, which will create a full-fledged django app for you. This can be accomplished with the command:
...
For the rest of this guide, the scriptly instance will be called scriptly_openshift. Next you will want to remove the old django project, called myproject and edit the *application* file and change all instances of myproject to YourProjectName(scriptly_openshift in this example).
      ...
      ::
            ...
            DATABASES = {
                'default': {
                    ...
                    'NAME': os.environ.get('DATABASE_NAME', 'scriptly'),
                    'USER': os.environ.get('DATABASE_USER', 'scriptly'),
                    'PASSWORD': os.environ.get('DATABASE_PASSWORD', 'scriptly'),
...
3. Setup the requirements.txt file. The bootstrapper provides a requirements.txt file that already has all the apps needed to run Scriptly. Just copy it from YourAppName/YourAppName/ to the top level directory of OpenShift (which has things like setup.py and openshiftlibs.py)
