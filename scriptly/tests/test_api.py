...
class TestJobStatus(mixins.ScriptFactoryMixin, ApiTestMixin, TransactionTestCase):
    def test_reports_when_job_is_complete(self):
        ...
        response = self.client.get(
            reverse("scriptly:api_job_status", kwargs={"job_id": job.id})
        )
...
class TestJobDetails(
    ...
    def test_job_details(self):
        ...
        response = self.client.get(
            reverse("scriptly:api_job_details", kwargs={"job_id": job.id})
        )
        data = response.json()
...
class TestScriptAddition(mixins.ScriptFactoryMixin, ApiTestMixin, TransactionTestCase):
    def test_can_add_new_scripts(self):
        ...
        response = self.client.post(
            reverse("scriptly:api_add_or_update_script"),
            data=payload,
        )
        data = response.json()
...
class TestScriptSubmission(
    ...
    def test_defaults_to_latest_script(self):
        ...
        response = self.client.post(
            reverse("scriptly:api_submit_script", kwargs={"slug": "update-script"}),
            data=payload,
            content_type="application/json",
        )
        data = response.json()
        self.assertTrue(data["valid"])
        job = ScriptlyJob.objects.get(id=data["job_id"])
    ...
    def test_can_submit_script_with_json(self):
        ...
        response = self.client.post(
            reverse(
                "scriptly:api_submit_script", kwargs={"slug": script_version.script.slug}
            ),
            data=payload,
            content_type="application/json",
        )
