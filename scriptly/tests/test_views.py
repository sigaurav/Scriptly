...
from ..views import scriptly_celery
from .. import views as scriptly_views
from .. import models
...
class CeleryViews(mixins.ScriptFactoryMixin, mixins.FileCleanupMixin, TestCase):
    def setUp(self):
        ...
        settings.SCRIPTLY_CELERY = False

    def test_celery_results(self):
        request = self.factory.get(reverse("scriptly:all_queues_json"))
        request.user = self.user
        response = scriptly_celery.all_queues_json(request)
        ...
        job.user = self.user
        job.status = models.ScriptlyJob.RUNNING
        job.save()
        response = scriptly_celery.all_queues_json(request)
        d = json.loads(response.content.decode("utf-8"))
    ...
    def test_celery_commands(self):
        ...
        request = self.factory.post(
            reverse("scriptly:celery_task_command"), celery_command
        )
        anon = AnonymousUser()
        request.user = anon
        response = scriptly_celery.celery_task_command(request)
        d = response.content.decode("utf-8")
        ...
        for i in ["resubmit", "rerun", "delete"]:
            ...
            request = self.factory.post(
                reverse("scriptly:celery_task_command"),
                celery_command,
            )
            request.user = self.user
            response = scriptly_celery.celery_task_command(request)
    ...
    def test_celery_task_view(self):
        ...
        view = scriptly_celery.JobView.as_view()
        request = self.factory.get(
            reverse("scriptly:celery_results", kwargs={"job_id": job.pk})
        )
    ...
    def test_celery_nonexistent_task(self):
        ...
        view = scriptly_celery.JobView.as_view()
        request = self.factory.get(
            reverse("scriptly:celery_results", kwargs={"job_id": "-1"})
        )
...
class ScriptlyViews(mixins.ScriptFactoryMixin, mixins.FileCleanupMixin, TestCase):
    def setUp(self):
        super(ScriptlyViews, self).setUp()
        self.factory = RequestFactory()
        self.script_view_func = scriptly_views.ScriptlyScriptView.as_view()
        self.json_view_func = scriptly_views.ScriptlyScriptJSON.as_view()
        self.user = factories.UserFactory()
        # the test server doesn't have celery running
        settings.SCRIPTLY_CELERY = False
    ...
    def test_multiple_choice_clone(self):
        ...
        job = utils.create_scriptly_job(
            ...
            data={
                ...
                "scriptly_type": script_version.pk,
            },
        )
        request = self.factory.post(
            reverse(
                "scriptly:scriptly_script_clone",
    ...
    def test_clone_job_into_specific_version(self):
        ...
        request = self.factory.get(
            reverse("scriptly:scriptly_script", kwargs=job_kwargs),
        )
        request.user = AnonymousUser()
        ...
        self.assertEqual(
            context["form"]["scriptly_form"]["scriptly_type"].value(), self.version1_script.pk
        )
    ...
    def test_multiple_choice(self):
        ...
        url = reverse("scriptly:scriptly_script", kwargs={"slug": script.slug})
        data = {
            "job_name": "abc",
            "scriptly_type": script_version.pk,
            "scriptly_parser": script_version.scriptparser_set.first().pk,
        ...
        job = models.ScriptlyJob.objects.latest("created_date")
        files = [
            i.value.name
    ...
    def test_form_groups(self):
        ...
        scriptly_form = forms["scriptly_form"]
        data.update(scriptly_form.initial)
        subparser = list(forms["parsers"].keys())[0][0]
        data["scriptly_parser"] = subparser

        url = reverse("scriptly:scriptly_script", kwargs={"slug": script_version.script.slug})
        request = self.factory.post(url, data=data)
    ...
    def test_job_view_permissions(self):
        ...
        # Make a new user
        user1 = User(username="scriptly")
        user1.save()
        user2 = User(username="scriptly2")
        user2.save()
        ...
        response.render()
        self.assertContains(
            response, models.ScriptlyJob.error_messages["invalid_permissions"]
        )

    def test_validates_chosen_subparser(self):
        # Addresses https://github.com/scriptly/Scriptly/issues/288
        script_version = self.subparser_script
        script = script_version.script
...
class WoeeyScriptSearchViews(
    ...
    def setUp(self):
        ...
        self.factory = RequestFactory()
        self.json_view_func = scriptly_views.ScriptlyScriptSearchJSON.as_view()
        self.json_html_view_func = scriptly_views.ScriptlyScriptSearchJSONHTML.as_view()
        # the test server doesn't have celery running
        settings.SCRIPTLY_CELERY = False
    ...
    def test_search_json_with_name(self):
        url = reverse("scriptly:scriptly_search_script_json")
    ...
    def test_search_json_html_with_name(self):
        url = reverse("scriptly:scriptly_search_script_jsonhtml")
        request = self.factory.get(url, data={"q": "1 name"})
        response = self.json_view_func(request)
        d = load_JSON_dict(response.content)
        self.assertEqual(len(d["results"]), 1)
...
class TestApiKeyViews(TestCase):
    ...
    def test_can_create_api_key(self):
        url = reverse("scriptly:create_api_key")
        request = self.request.post(url, data={"name": "test-key"})
        request.user = factories.UserFactory()
        response = scriptly_views.create_api_key(request)
    ...
    def test_can_toggle_api_key(self):
        ...
        url = reverse("scriptly:toggle_api_key", kwargs={"id": api_key.id})
        request = self.request.post(url)
        request.user = factories.UserFactory()
        response = scriptly_views.toggle_api_key(request, id=api_key.id)
    ...
    def test_cant_delete_api_key_if_wrong_user(self):
        ...
        url = reverse("scriptly:delete_api_key", kwargs={"id": api_key.id})
        request = self.request.delete(url)
        new_profile = factories.ProfileFactory(user__username="foo")
        request.user = new_profile.user
        response = scriptly_views.delete_api_key(request, id=api_key.id)
        self.assertEqual(response.status_code, 404)
...
class TestProfileView(TestCase):
    def test_doesnt_show_settings_if_not_logged_in_user(self):
        ...
        user = factories.UserFactory()
        url = reverse("scriptly:profile", kwargs={"username": user.username})

        request = request_factory.get(url)
        request.user = other_user
        view = scriptly_views.ScriptlyProfileView.as_view()
...
class TestHomeView(mixins.ScriptFactoryMixin, mixins.FileCleanupMixin, TestCase):
    def test_sorts_scripts_by_name_and_favorite(self):
        request_factory = RequestFactory()
        user = factories.UserFactory()
        url = reverse("scriptly:scriptly_home")
        request = request_factory.get(url)
        request.user = user

        # by default, we sort by name
        view = scriptly_views.ScriptlyHomeView.as_view()
