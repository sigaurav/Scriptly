...
from ..models import ScriptParameterGroup, ScriptParameter, ScriptVersion, ScriptlyJob
...
class FormTestCase(
    mixins.ScriptFactoryMixin, mixins.FileCleanupMixin, TransactionTestCase
):
    @staticmethod
    def get_mvdict(data):
    ...
    def test_master_form(self):
        ...
        self.assertTrue(isinstance(form, ScriptlyForm))
        qdict = self.get_mvdict(config.SCRIPT_DATA["translate"].get("data"))
        utils.validate_form(
            form=form, data=qdict, files=config.SCRIPT_DATA["translate"].get("files")
        )
        self.assertTrue(form.is_valid())

    def test_groups_dont_duplicate(self):
        # fixes https://github.com/scriptly/Scriptly/issues/163
        ...
        script_path = test_utils.save_script_path(
            os.path.join(config.SCRIPTLY_TEST_SCRIPTS, "translate2.py")
        )
        new_translate = utils.add_scriptly_script(
        ...
        for new_parsers, old_parsers in zip(new_form["parsers"], old_form["parsers"]):
            for new_subparser, old_subparser in zip(
                new_form["parsers"][new_parsers], old_form["parsers"][old_parsers]
            ):
                self.assertEqual(
                    new_subparser["group_name"], old_subparser["group_name"]
                )
    ...
    def test_group_form(self):
        ...
        form = utils.get_form_groups(script_version=script_version)
        self.assertEqual(
            len(form["parsers"][subparser_key]),
            1,
            "Script Parameter group is hidden but shown",
        )
        group.hidden = False
        group.save()
        form = utils.get_form_groups(script_version=script_version)
        self.assertEqual(
            len(form["parsers"][subparser_key]),
            2,
            "Script Parameter group is shown but hidden",
        )
        ...
        self.assertNotIn(
            slug,
            form["parsers"][subparser_key][1]["form"].fields,
            "Script Parameter is hidden but shown",
        )
        param.hidden = False
        param.save()
        form = utils.get_form_groups(script_version=script_version)
        self.assertIn(
            slug,
            form["parsers"][subparser_key][1]["form"].fields,
            "Script Parameter is shown but hidden",
        )
    ...
    def test_multiplechoice_form(self):
        ...
        form_str = str(form)
        self.assertTrue(forms_config.SCRIPTLY_MULTI_WIDGET_ANCHOR in form_str)
        self.assertTrue(forms_config.SCRIPTLY_MULTI_WIDGET_ATTR in form_str)
        ...
        for file_key, files in fdict.items():
            ...
            for file_location in files:
                ...
                uploaded_files[file_key].append(
                    SimpleUploadedFile(file_name, file_location.read())
                )

        utils.validate_form(form=form, data=qdict, files=uploaded_files)
        self.assertTrue(form.is_valid())
        ...
        job = utils.create_scriptly_job(
            script_version_pk=script_version.pk, data=form.cleaned_data
        )
        # check the files are here
        file_param = test_utils.get_subparser_form_slug(
            script_version, "multiple_file_choices"
        )
        files = [
            i.value for i in job.get_parameters() if i.parameter.form_slug == file_param
        ]
    ...
    def test_without_args_form(self):
        ...
        form = utils.get_master_form(script_version=script_version)
        # check our wrapper is in the form render
        # in a normal POST request, the data will contain {'scriptly_type': script_pk}. This will cause the is_bound
        ...
        # get the job again
        job = ScriptlyJob.objects.get(pk=job.pk)
        self.assertEqual(job.status, ScriptlyJob.COMPLETED)

    def test_scriptly_form(self):
        # Make sure scriptly form exists and has everything we need in it
        script_version = self.without_args
        forms = utils.get_form_groups(script_version=self.without_args)
        scriptly_form = forms["scriptly_form"]
        self.assertDictEqual(scriptly_form.initial, {"scriptly_type": script_version.pk})

    def test_multiple_file_field_initial_value(self):
        # Addresses https://github.com/scriptly/Scriptly/issues/248
        script_version = self.choice_script
        multiple_files_param = script_version.scriptparameter_set.get(
            script_param="multiple_file_choices"
        )
        ...
        form = utils.get_form_groups(
            script_version=script_version,
            initial_dict={multiple_files_param.form_slug: ["file1", "file2"]},
        )
        # TODO: Make a function to ease this
        initial_files = [
            i.path
            for i in form["parsers"][
                (multiple_files_param.parser.pk, multiple_files_param.parser.name)
            ][1]["form"]
            .fields[multiple_files_param.form_slug]
            .initial
        ]
    ...
    def test_form_with_custom_widget(self):
        ...
        widget = factories.ScriptlyWidgetFactory(
        ...
        field = form["parsers"][(choice_param.parser.pk, choice_param.parser.name)][1][
            "form"
    ...
    def test_handles_special_characters(self):
        script_version = self.choice_script
        # Associate a custom widget with a field
        choice_str_slug = test_utils.get_subparser_form_slug(
            script_version, "choices_str"
        )
        choice_str = "This & is special"
        form = utils.get_master_form(script_version=script_version)
        qdict = self.get_mvdict(
            {
                choice_str_slug: [choice_str],
