...
from ..models import ScriptVersion, ScriptlyFile, ScriptlyJob
...
class FileCleanupMixin(object):
    def tearDown(self):
        for i in ScriptlyFile.objects.all():
            try:
        ...
        for i in ScriptlyJob.objects.all():
...
class ScriptFactoryMixin(ScriptTearDown, object):
    def setUp(self):
        ...
        self.version2_script_path = os.path.join(
            config.SCRIPTLY_TEST_SCRIPTS, "versioned_script", "v2.py"
        )
...
class FileMixin(object):
    def setUp(self):
        self.storage = utils.get_storage(local=not scriptly_settings.SCRIPTLY_EPHEMERAL_FILES)
        self.filename_func = lambda x: os.path.join(scriptly_settings.SCRIPTLY_SCRIPT_DIR, x)
