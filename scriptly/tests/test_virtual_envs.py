...
from scriptly.backend.utils import create_scriptly_job
from scriptly.models import ScriptlyJob
from scriptly.tasks import setup_venv
...
class TestVirtualEnvironments(TransactionTestCase):
    ...
    def test_reuses_virtual_env(self):
        ...
        (venv_executable, stdout, stderr, return_code) = setup_venv(venv)
        self.assertTrue(os.path.exists(venv_executable))
        with mock.patch("scriptly.tasks.run_and_stream_command") as command_runner:
    ...
    def test_installs_pip(self):
        venv = self.venv
        setup_venv(venv)
        if scriptly_settings.IS_WINDOWS:
            self.assertTrue(
                os.path.exists(
    ...
    def test_job_can_run_in_venv(self):
        ...
        pandas_script_path = os.path.join(
            config.SCRIPTLY_TEST_SCRIPTS, "venv_pandas_test.py"
        )
        ...
        pandas_script.save()
        job = create_scriptly_job(
            script_version_pk=pandas_script_version.pk,
            data={
                "job_name": "abc",
            },
        )
        self.assertEqual(job.status, ScriptlyJob.SUBMITTED)
