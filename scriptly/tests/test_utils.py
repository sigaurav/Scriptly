...
class TestUtils(mixins.ScriptFactoryMixin, mixins.FileMixin, TransactionTestCase):
    ...
    def test_anonymous_users(self):
        ...
        scriptly_settings.SCRIPTLY_ALLOW_ANONYMOUS = False
        d = utils.valid_user(script, user)
    ...
    def test_valid_user(self):
        ...
        scriptly_settings.SCRIPTLY_SHOW_LOCKED_SCRIPTS = False
        d = utils.valid_user(script, user)
    ...
    def test_job_file_outputs(self):
        ...
        from scriptly.models import ScriptlyJob, UserFile

        script = os.path.join(config.SCRIPTLY_TEST_SCRIPTS, "file_maker.py")
        with open(script) as o:
            new_file = self.storage.save(self.filename_func("file_maker.py"), o)
        res = utils.add_scriptly_script(script_path=new_file, group=None)
        ...
        for job_file in UserFile.objects.filter(job=job):
            scriptly_file = job_file.system_file
            self.assertEqual(
                os.path.getsize(self.storage.path(scriptly_file.filepath.name)),
                scriptly_file.size_bytes,
            )

        # Check for the zip file overwrite in for https://github.com/scriptly/Scriptly/issues/202
    ...
    def test_duplicate_scriptversion_checksums(self):
        ...
        script = os.path.join(config.SCRIPTLY_TEST_SCRIPTS, "versioned_script", "v1.py")
        with open(script) as o:
            v1 = self.storage.save(self.filename_func("v1.py"), o)
        res = utils.add_scriptly_script(script_path=v1, script_name="test_versions")
    ...
    def test_add_scriptly_script(self):
        from scriptly.models import ScriptParser, ScriptVersion
        ...
        self.assertEqual(
            str(res["errors"]), ScriptVersion.error_messages["duplicate_script"]
        )
        dup_first_version = res["script"]
        self.assertEqual(first_version.script_version, dup_first_version.script_version)
        ...
        # between scripts
        script = os.path.join(config.SCRIPTLY_TEST_SCRIPTS, "versioned_script", "v2.py")
        with open(script) as o:
            v2 = self.storage.save(self.filename_func("v2.py"), o)

        res = utils.add_scriptly_script(script_path=v2, script_name="test_versions")
        ...
        script = os.path.join(config.SCRIPTLY_TEST_SCRIPTS, "versioned_script", "v3.py")
        with open(script) as o:
            v3 = self.storage.save(self.filename_func("v3.py"), o)

        res = utils.add_scriptly_script(script_path=v3, script_name="test_versions")
...
class TestFileDetectors(TestCase):
    def test_detector(self):
        self.file = os.path.join(config.SCRIPTLY_TEST_DATA, "fasta.fasta")
    ...
    def test_delimited(self):
        self.file = os.path.join(config.SCRIPTLY_TEST_DATA, "delimited.tsv")
