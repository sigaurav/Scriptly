...
from scriptly import settings as scriptly_settings
from scriptly.backend.utils import add_scriptly_script
from scriptly.models import (
    ScriptlyJob,
)
from scriptly.tasks import (
    cleanup_dead_jobs,
    get_latest_script,
)
...
class TaskTests(mixins.ScriptFactoryMixin, TestCase):
    def test_job_cleanup(self):
        from ..models import ScriptlyJob
        from ..tasks import cleanup_scriptly_jobs
        import time
        ...
        user_job.user = user
        user_job.save()
        scriptly_settings.SCRIPTLY_JOB_EXPIRATION.update(
            {
                "user": timedelta(hours=1),
                "anonymous": timedelta(hours=1),
            }
        )
        cleanup_scriptly_jobs()
        self.assertListEqual(list(ScriptlyJob.objects.all()), [anon_job, user_job])
        time.sleep(0.1)
        scriptly_settings.SCRIPTLY_JOB_EXPIRATION.update(
            {
                "user": timedelta(hours=1),
        ...
        cleanup_scriptly_jobs()
        self.assertListEqual(list(ScriptlyJob.objects.all()), [user_job])
        ...
        cleanup_scriptly_jobs()
        self.assertListEqual(list(ScriptlyJob.objects.all()), [])
...
class TestGetLatestScript(mixins.FileMixin, mixins.ScriptTearDown, TestCase):
    def setUp(self):
        ...
        res = add_scriptly_script(script_path=v1, script_name="test_versions")
    ...
    def test_get_latest_script_downloads_new_script(self):
        ...
        res = add_scriptly_script(script_path=v2, script_name="test_versions")
...
class TestCleanupDeadJobs(mixins.ScriptFactoryMixin, TestCase):
    def test_handles_unresponsive_workers(self):
        ...
        with mock.patch("scriptly.tasks.celery_app.control.inspect") as inspect_mock:
            running_job = factories.generate_job(self.translate_script)
            running_job.status = ScriptlyJob.RUNNING
            running_job.save()
            ...
            cleanup_dead_jobs()
            self.assertEqual(
                ScriptlyJob.objects.get(pk=running_job.id).status, ScriptlyJob.RUNNING
            )
    ...
    def test_cleans_up_dead_jobs(self):
        ...
        dead_job.status = ScriptlyJob.RUNNING
        dead_job.save()
        active_job = factories.generate_job(self.translate_script)
        active_job.status = ScriptlyJob.RUNNING
        ...
        with mock.patch("scriptly.tasks.celery_app.control.inspect") as inspect_mock:
            ...
            self.assertEqual(
                ScriptlyJob.objects.get(pk=dead_job.id).status, ScriptlyJob.FAILED
            )
            self.assertEqual(
                ScriptlyJob.objects.get(pk=active_job.id).status, ScriptlyJob.RUNNING
            )
