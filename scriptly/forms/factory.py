...
from . import config
from .scripts import ScriptlyForm
    ...
    def render(name, value=None, attrs=None, renderer=None):
        ...
        pieces = [
            "<{tag} {multi_attr}>{widget}</{tag}>".format(
                tag="div",
                multi_attr=config.SCRIPTLY_MULTI_WIDGET_ATTR,
        ...
        pieces.append(
            format_html(
                ...
                anchor=config.SCRIPTLY_MULTI_WIDGET_ANCHOR,
                data=data_attrs,
            )
        )
        return mark_safe("\n".join(pieces))
...
class ScriptlyFormFactory(object):
    ...
    def get_group_forms(self, script_version=None, initial_dict=None, render_fn=None):
        ...
        if initial_dict.get("scriptly_parser") is not None:
            form_initial["scriptly_parser"] = initial_dict["scriptly_parser"]
        scriptly_form = ScriptlyForm(initial=form_initial)
        script_info = {
            "action": script_version.get_url(),
            "parsers": OrderedDict(),
            "scriptly_form": scriptly_form,
        }
        ...
        for parser, group_map in parser_group_map.items():
            parser_pk = parser[0]
            if (
                scriptly_form.fields["scriptly_parser"].initial is None
                and parser_pk is not None
            ):
                scriptly_form.fields["scriptly_parser"].initial = parser_pk
    ...
    def get_master_form(self, script_version=None, pk=None, parser=None):
        ...
        master_form = ScriptlyForm(initial={"scriptly_type": pk})
...
DJ_FORM_FACTORY = ScriptlyFormFactory()
