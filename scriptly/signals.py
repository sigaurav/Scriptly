...
def task_completed(sender=None, **kwargs):
    ...
    job_id = task_kwargs.get("scriptly_job")
    # Just return if it is not a scriptly_job!
    if not job_id:
        return
    ...
    if state and job.status not in ScriptlyJob.TERMINAL_STATES:
        job.status = ScriptlyJob.COMPLETED if state == states.SUCCESS else state
