...
def job_status(request, job_id):
    job = models.ScriptlyJob.objects.get(id=job_id)
    if job.can_user_view(request.user):
        return JsonResponse(
            {
                "status": job.status,
                "is_complete": job.status in models.ScriptlyJob.TERMINAL_STATES,
            }
        )
    else:
        return JsonResponse(
...
def job_details(request, job_id):
    ...
    if job.can_user_view(request.user):
        assets = []
        is_terminal = job.status in models.ScriptlyJob.TERMINAL_STATES
        if is_terminal:
