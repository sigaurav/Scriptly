...
def create_argparser(script_version):
    ...
    This is meant to assist in processing a command sent to the Scriptly API for
    ...
    be an error.
    """
    parser = argparse.ArgumentParser(prog="scriptly-temp")
    subparsers = None
    ...
    def log_error(parser):
        ...
        def inner(message):
            parser._scriptly_error = message
    ...
    for subparser_name, arguments in grouped_parameters.items():
        if subparser_name:
            if not subparsers:
                subparsers = parser.add_subparsers(dest="scriptly_subparser")
...
def submit_script(request, slug=None):
    ...
    if valid:
        ...
        try:
            ...
            return JsonResponse(
                {"valid": False, "errors": {"command": parser._scriptly_error}}, status=400
            )
        if valid and group_valid:
            ...
            subparser_id = script_version.scriptparser_set.get(
                name=job_data.pop("scriptly_subparser", "")
            ).id
            form = utils.get_master_form(
                script_version=script_version, parser=subparser_id
            )
            scriptly_form_data = job_data.copy()
            scriptly_form_data["scriptly_type"] = script_version.pk

            # We need to remap uploaded files to the correct slug
            form_slugs = list(scriptly_form_data)
            for form_slug in form_slugs:
                form_value = scriptly_form_data[form_slug]
                if isinstance(form_value, list):
                    ...
                    if form_value in files:
                        ...
                        scriptly_form_data[form_slug] = [""]

            utils.validate_form(form=form, data=scriptly_form_data, files=files)
...
def add_or_update_script(request):
    ...
    data = form.cleaned_data
    group = data["group"] or scriptly_settings.SCRIPTLY_DEFAULT_SCRIPT_GROUP

    response = []

    for script_name, script_file in files.items():
        script_path = utils.default_storage.save(
            os.path.join(scriptly_settings.SCRIPTLY_SCRIPT_DIR, script_file.name),
            script_file,
        )
        ...
        results = utils.add_scriptly_script(**add_kwargs)
        output = {
            "script": script_name,
